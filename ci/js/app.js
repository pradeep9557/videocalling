...
function initializeClient() {
  ...
  $.getJSON('/videos/app_id', function(json) {
    joinData = json;
  ...
  // Get the App Id from the node.js server
    client.init(appId, function (obj) {
      ...
      joinChannel(appId, token, channel, uid);      // Tutorial Step 2
...
// Tutorial Step 2
function joinChannel(appId, token, channel, uid) {
...
function onEndCallButtonClicked() {
  ...
  $.post("/videos/stop_recording", payload, function(e) {
    console.log("Stoping recording..",e);
     localStream.stop();
    localStream.close();
...
function startRecording() {
  ...
  console.log(payload);
  $.post("/videos/start_recording", payload, function(e) {
    console.log("started recording..",e);
    var data = JSON.parse(e)
    joinData.sid = data.sid;
    joinData.resourceId = data.resourceId;
